...
public class SimpleCapsuleWithStickMovement : MonoBehaviour
{
	...
	void StickMovement()
	{
		...
		if(primaryAxis != Vector2.zero)
		{
			...
			_fc.IsShowing.Value = false; // 追尾UIを隠す
			Clipboard.Instance.IsShowing.Value = false; // ボードも隠す
		}
		else
		{
			_as.Stop();
			_fc.IsShowing.Value = true; // 追尾UIを見せる
			Clipboard.Instance.IsShowing.Value = true; // ボードも見せる
		}
